...
down_revision: Union[str, None] = '9a0623c36ae4'
...
def upgrade() -> None:
    ...
    key_value_table = sa.sql.table('key_value',
    ...
    old_info = [{'user_id': r[0], 'key': 'app.dashboards', 'value': r[1], 'hash': r[2]} for r in results]
...
def downgrade() -> None:
    ...
    conn = op.get_bind()
    res = conn.execute(sa.text("SELECT user_id, value, hash FROM key_value WHERE key='app.dashboards'"))
    results = res.fetchall()
